  ...
  module Generators
    class RouterGenerator < Rails::Generators::NamedBase
      ...
      def create_view_files
        actions.each do |action|
          @action = action
          @template_path = File.join(backbone_path,"templates", plural_name, "#{action}.hbs")

          template "template.hbs", @template_path

          if options[:coffee]
            template "view.coffee", File.join(backbone_path, "views", plural_name, "#{action}_view.js.coffee")
          else
            template "view.js", File.join(backbone_path, "views", plural_name, "#{action}_view.js")
          end
         end
      end
